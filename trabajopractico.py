...
while True:
    ...
    if agrupacion_elegida.lower() == 'salir':
        ...
        df_agrupacion = result_1v[result_1v['agrupacion_nombre'] == agrupacion_elegida]
        resultados_presidente = df_agrupacion[df_agrupacion['cargo_nombre'] == 'PRESIDENTE Y VICE']
        df_provincia_votos = resultados_presidente.groupby('distrito_nombre')['votos_cantidad'].sum().nlargest(5)
        otras_agrupaciones = [a for a in agrupaciones_permitidas if a != agrupacion_elegida]
        df_otras_agrupaciones = result_1v[result_1v['agrupacion_nombre'].isin(otras_agrupaciones)]
